...
public class ARPPing implements DiscoveryMethod {
    ...
    private void triggerArp(String host, int timeout) {
        ...
        try {
            CompletableFuture.allOf(futures).get(quickTimeout + 100, java.util.concurrent.TimeUnit.MILLISECONDS);
        } catch (Exception ignored) {
        }
    }
    ...
    public void batchTriggerArp(List<String> hosts, int timeoutMs) {
        ...
        // Wait for all triggers (with overall timeout)
        try {
            CompletableFuture.allOf(allFutures.toArray(new CompletableFuture[0]))
                .get(timeoutMs + 1000, java.util.concurrent.TimeUnit.MILLISECONDS);
        } catch (Exception ignored) {
        }
