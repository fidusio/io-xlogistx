...
public class NIOHTTPServer
    ...
    public final static AppVersionDAO VERSION = new AppVersionDAO("NOYFB::2.2.4");
            ...
            try {
                if (hph.parseRequest(inBuffer)) {
                    ...
                    if (!httpsRedirect()) {
                        ...
                        if (logger.isEnabled())
                            logger.getLogger().info(SUS.toCanonicalID(':', "http", getRemoteAddress(), hph.getRequest(true) != null ? hph.getRequest(true).getURI() : ""));
                    }
                } else {
