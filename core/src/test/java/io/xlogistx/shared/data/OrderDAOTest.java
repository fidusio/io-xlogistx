...
public class OrderDAOTest {
  ...
  public void testOrderDAO() {
    ...
    assertEquals(new BigDecimal("500.00"), total.getAmount());
    ...
    orderDAO.getOrderItems().add(orderItemDAO1);
    orderDAO.getOrderItems().add(orderItemDAO2);
    orderDAO.getOrderItems().add(orderItemDAO3);
    orderDAO.setOrderStatus(XXStatus.PENDING);
    orderDAO.setCreationTime(System.currentTimeMillis());
    orderDAO.setLastTimeUpdated(System.currentTimeMillis());

    assertEquals(3, orderDAO.getOrderItems().size());

    AmountDAO orderTotal = XXDataUtil.computeTotal(orderDAO);
    assertNotNull(orderTotal);
    assertNotNull(orderTotal.getAmount());
    assertEquals("$2200.00", orderTotal.toString());

    assertNull(orderDAO.getTotal());
    XXDataUtil.updateTotal(orderDAO);
    assertNotNull(orderDAO.getTotal());
    assertEquals("$2200.00", orderDAO.getTotal().toString());
  }
